---
title: "HaloPSA 09: Claude access architecture (OAuth MCP worker, employee rollout, SARA)"
type: runbook
updated: 2026-09-09
tags: [halopsa, mcp-connector, cloudflare, claude-access]
related: [runbooks/halopsa-01-overview, projects/halopsa/STATUS]
source: "HaloPSA Claude Project: claude/HaloPSA-Runbook-09-Claude-Access-Architecture.md"
migrated: 2026-09-09
---
> Migrated 2026-09-09 from the HaloPSA Claude Project (`HaloPSA-Runbook-09-Claude-Access-Architecture.md`). This file is now the canonical copy; the project doc is frozen. The verbatim original is in `projects/halopsa/archive/project-export-2026-09-09/`. Em and en dashes in prose were replaced per CLAUDE.md; code blocks are untouched. Cross-references were rewritten to brain paths.
...
> **Last updated:** 2026-09-09 (§11.18 - user impersonation granted to Operations and Sales
> Executive so they can approve quotes on a client user's behalf. Prior: §11.17 - post-go-live incident 4: Meghan "Ticket not found"
...
### 11.18 Change: user impersonation for Operations and Sales Executive (applied 2026-09-09)

**Request (Ryan, 2026-09-09):** Ops and Sales exec groups need to impersonate client users so
they can approve quotes on the client's behalf when a quote was sent to the client directly.

**Change:** `Can impersonate Users: Not set -> Yes` on **Operations**
(`70222833-6e9f-4ebe-951a-c55f04d35fce`, id_int 22) and **Sales Executive**
(`febac04c-a0f4-4c36-bc6e-549c06dcb710`, id_int 8). Applied via Chrome on the role's
Permissions tab, saved, view-mode diffed against the pre-change snapshot: exactly one line
changed on each role (Ops line 255 of 256, SE line 277 of 278). Sales Rep, Executive and
Finance untouched (Ryan chose Sales Executive over the vacant Sales Rep role).

**API verification:** `Agent/15` (Soltis, Operations) and `Agent/25` (Schilling, Sales
Executive) `roles[].claims[]` now both carry `{"type":"Iu","value":"1"}`. **New claim code
resolved: `Iu` = Can impersonate Users** (add to the §6 claim map).

**Rollback:** same field back to `Not set` (or `No`; Halo treats them the same) on each role.

**Halo facts learned:**
- The Permissions edit form lazy-renders sections as they scroll into view, so `scroll_to`
  a react-select ref, wait ~2 s, then click the ref and type the option + Enter. The first
  scroll_to often lands on a half-rendered page; verify with a JS read of the
  `singleValue` text before saving. Clicking an option by screenshot coordinates is
  unreliable because the layout shifts while sections render.
- Cheap pre-save safety check: build a label->value map from the view-mode snapshot and
  compare it against every `label` + `singleValue`/`placeholder` in the edit form. Only
  render-only differences should appear (Suppliers rows, Client Group Override and
  Conversation access level show "Select..." for Not set).
- Impersonation is a broad grant (act as any end user in the portal, not only on quotes).
  Narrower alternatives were not evaluated; revisit if the AI-risk audit (§5/§6) is rerun.
